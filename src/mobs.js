...
(function () {
  ...
  const M = CF.mobs = {
    ...
    trySpawnAt(type, x, fy, z, opts) { // rule-gated spawn (1.12 light/grass/distance/cap). Returns mob|null.
      ...
      if (T.passive && M.passiveCount() >= PASSIVE_CAP) return null;
  ...
  CF.mobTick = (dt = 0.05) => {
    ...
    for (const m of M.list.slice()) {
      ...
      if (m.dead) { M.remove(m); continue; }
      if (P && T.hostile) { // passives are persistent (1.12 animals don't despawn); hostiles despawn far
    ...
    if (CF.survival && M.sched !== false && P) {
      ...
      // passive natural spawn: grass, daylight, sparse (1.12 animals are rare + persistent)
      if (CF.dayFactor && CF.dayFactor() > 0.7 && M.passiveCount() < PASSIVE_CAP && Math.random() < 0.15) {
        const ang = Math.random() * Math.PI * 2, rr = 24 + Math.random() * 16;
        const x = Math.floor(P.pos[0] + Math.cos(ang) * rr), z = Math.floor(P.pos[2] + Math.sin(ang) * rr);
        W.ensureAround(x, z, 0);
        const fy = W.heightAt(x, z) + 1;
        if (W.get(x, fy - 1, z)) W.ensureLight(x >> 4, z >> 4);
        const r = Math.random(), type = r < 0.34 ? 'pig' : r < 0.67 ? 'cow' : 'sheep';
        M.trySpawnAt(type, x + 0.5, fy, z + 0.5);
      }
    }
    M.breedScan();
