...
(function () {
  ...
  const M = CF.mobs = {
    ...
    spawn(type, x, y, z, opts) { // y = FEET level. Raw create (no rules) - shots/tests use this.
      ...
      const m = { uid: M.nextUid++, type, pos: [x, y + T.h / 2, z], vel: [0, 0, 0], hp: T.hp, onGround: false, burnT: 0, burning: false, atkCd: 0, path: null, repathT: 0, fuse: 0, shootCd: 0, swell: 0 };
    ...
    hurt(m, n, src) {
      ...
      if (m.hp <= 0) {
        m.dead = true;
        if (src === 'player' && CF.give) for (const loot of M.rollDrop(m.type)) CF.give(loot.name, loot.n); // Tier-1: no item entities; player kills -> inventory
        M.remove(m);
      }
    },
  ...
  CF.mobTick = (dt = 0.05) => {
    ...
    // 1.12 hostile mix (approx): zombie 50% / skeleton 30% / creeper 20%.
    if (CF.survival && M.sched !== false && P && M.hostileCount() < HOSTILE_CAP) {
      ...
      while (budget-- > 0) {
        ...
        const r = Math.random(), type = r < 0.5 ? 'zombie' : r < 0.8 ? 'skeleton' : 'creeper';
        M.trySpawnAt(type, x + 0.5, fy, z + 0.5);
      }
    }
    projectileTick(dt);
  ...
  CF.buildMobVerts = (cam) => {
    ...
    for (const m of M.list) {
      ...
      for (const b of CF.MOBS[m.type].boxes) {
        let col = C[b.color] || 0;
        if (swell > 0 && (swell > 0.6 ? ((CF.ticks & 1) ? 1 : 0) : 0)) col = C.creeFlash; // creeper pre-blast white flash
        const g = 1 + swell * 0.35; // swelling creeper puffs up
